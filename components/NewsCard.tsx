    ...
    >
      ...
      <div className="px-6 pt-6 pb-2 flex items-start justify-between">
        <div className="flex items-center gap-3.5">
          ...
          >
             <div className="w-full h-full rounded-[10px] overflow-hidden">
                <img 
                  src={`https://picsum.photos/seed/source${data.id}/100`} 
          ...
          <div>
            ...
            >
              ...
              <CheckCircle2 size={13} className="text-palette-red fill-palette-red/10" />
            </div>
        ...
        <button 
          ...
          className="w-8 h-8 flex items-center justify-center text-palette-tan/30 hover:bg-palette-beige/30 hover:text-gray-900 rounded-full transition-colors"
        >
          <span className="material-symbols-rounded" style={{ fontSize: '24px' }}>more_horiz</span>
        </button>
      </div>
      ...
      <div className="px-6 py-2">
        {data.type !== NewsType.EMBED && (
          <h3 className="text-[22px] font-[900] text-gray-900 tracking-tight leading-snug group-hover/card:text-palette-red transition-colors">
            {data.title}
          </h3>
        )}
      </div>
      ...
      {/* INTERACTION BAR */}
      <div className="mt-4 px-6 py-4 bg-palette-beige/10 border-t border-palette-beige/40 backdrop-blur-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button 
              onClick={(e) => { e.stopPropagation(); setIsLiked(!isLiked); }} 
              className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 group/like ${
                isLiked 
                ? 'bg-palette-red/5 text-palette-red shadow-sm shadow-palette-red/10' 
                : 'hover:bg-white hover:text-palette-red hover:shadow-sm text-palette-tan/60'
              }`}
            >
              <span 
                className={`material-symbols-rounded transition-transform duration-300 group-active/like:scale-125 ${isLiked ? 'animate-bounce-subtle' : ''}`}
                style={{ 
                  fontSize: '22px', 
              ...
              <span className="text-xs font-black tracking-tight">{(isLiked ? data.likes + 1 : data.likes).toLocaleString()}</span>
            </button>

            <button 
              onClick={(e) => { e.stopPropagation(); setShowComments(!showComments); }}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all duration-300 group/comment ${
                showComments 
                ? 'bg-gray-900/5 text-gray-900 shadow-sm shadow-gray-900/10' 
                : 'hover:bg-white hover:text-gray-900 hover:shadow-sm text-palette-tan/60'
              }`}
            >
              <span 
              ...
              <span className="text-xs font-black tracking-tight">{data.comments.toLocaleString()}</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-300 ${
                ...
                : 'text-palette-tan/30 hover:bg-white hover:text-gray-900 hover:shadow-sm'
              }`}
            >
              <span 
                className="material-symbols-rounded" 
            ...
            <button 
              onClick={(e) => e.stopPropagation()} 
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-palette-beige hover:bg-gray-900 hover:text-white text-palette-tan transition-all duration-300 active:scale-95 group/share"
            >
              <span className="material-symbols-rounded" style={{ fontSize: '18px' }}>ios_share</span>
              <span className="text-[10px] font-black uppercase tracking-wider">Paylaş</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {showComments && (
        <div onClick={(e) => e.stopPropagation()} className="border-t border-palette-beige/40 animate-in fade-in slide-in-from-top-2 duration-300">
          <CommentSection />
        </div>
      )}
    </div>
