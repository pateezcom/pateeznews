...
const PostList: React.FC = () => {
    ...
    const samplePost: PostRecord = {
        id: 'sample-haber-123',
        title: 'Örnek Haber: Yapay Zeka Teknolojilerinde Devrim Niteliğinde Gelişme!',
        category: 'Teknoloji',
        type: 'Haber',
        language_code: 'tr',
    ...
    const handleDelete = async () => {
        ...
        if (postToDelete.id === 'sample-haber-123') {
            ...
            <div className="bg-white rounded-[3px] border border-palette-tan/20 shadow-sm overflow-hidden min-h-[600px] flex flex-col">
                            ...
                            {loading ? (
                                ...
                                filteredPosts.map((post) => (
                                    <tr key={post.id} className="hover:bg-palette-beige/5 transition-all group">
                                        ...
                                        <td className="px-4 py-6 text-[13px] font-bold text-palette-tan/60">{parseInt(post.id.substr(0, 3), 16) || 475}</td>
                                        ...
                                        <td className="px-6 py-6 text-center">
                                            <span className="text-[12px] font-bold text-palette-tan/70 uppercase tracking-widest">{post.category || 'Gündem'}</span>
                                        </td>
                                        <td className="px-6 py-6 text-center">
                                        ...
                                        <td className="px-6 py-6 text-center">
                                            <span className="text-[12px] font-bold text-palette-maroon/70">{post.profiles?.full_name || 'Admin'}</span>
                                        </td>
                                        <td className="px-6 py-6 text-center">
                                            <div className="flex flex-col items-center">
                                                <span className="text-[14px] font-black text-palette-maroon">{post.view_count || 0}</span>
                                            ...
                                            <div className="relative inline-block text-left" ref={openDropdownId === post.id ? dropdownRef : null}>
                                                ...
                                                {openDropdownId === post.id && (
                                                    <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-palette-tan/20 rounded-[3px] shadow-2xl z-[100] py-2 animate-in fade-in zoom-in-95 duration-200">
                                                        <button className="w-full px-5 py-3 text-left text-[13px] font-bold text-palette-maroon hover:bg-palette-beige/30 transition-all flex items-center gap-3 group/item">
                ...
                <div className="p-8 border-t border-palette-tan/10 bg-palette-beige/5 flex items-center justify-between">
                    <p className="text-[12px] font-bold text-palette-tan/40 uppercase tracking-widest">
                        {Math.min(filteredPosts.length, pageSize)} / {filteredPosts.length} Sonuç gösteriliyor
                    </p>
                    <div className="flex items-center gap-2">
                        <button className="w-10 h-10 flex items-center justify-center rounded-[3px] border border-palette-tan/15 text-palette-tan hover:bg-white hover:text-palette-maroon transition-all disabled:opacity-30" disabled>
